...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <!--
    page2gly.xsl is a proof-of-concept stylesheet for
    generating GLY (JSesh source) files from our XML pages.
    It should prove that our XML structure is a sufficient
    source of truth for our site's annotations.

    Note that, to get the text format just right, I'm very
    particular with the whitespace inside the templates,
    and the newlines are significant!

    Example use:
      saxon -xsl:tools/xslt/page2gly.xsl -s:src/iwefaa.xml
        -o:iwefaa.gly
  -->
